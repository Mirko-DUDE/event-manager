...
- **Debito prestazioni search** *(bassa priorità, da valutare in fase di ottimizzazione post-sviluppo)*: la ricerca usa `contains` → regex MongoDB (`/query/i`) senza indice — collection scan su tutti i documenti attivi. Con ~3000 contatti stimati è nell'ordine dei 10–50ms lato DB e 100–300ms percepiti, accettabile. Se risultasse lenta in produzione: aggiungere indice testuale MongoDB su `firstName`, `lastName`, `email` e sostituire `contains` con `$text` in Payload (o query raw). Non implementare ora.
