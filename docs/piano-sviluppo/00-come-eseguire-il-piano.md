...
- **Aggiornare `docs/piano-sviluppo/CHANGELOG.md`** con una voce sotto `[Unreleased]` che descrive cosa è stato fatto, inclusi esiti di test (positivi o negativi) — vedi `08-changelog-commit.mdc` per la checklist completa di pre-commit.
...
- **Fase 3**: Fase 2 chiusa (✅ su tutte le sottofasi in `fase-2-login.md`). Prima di iniziare, avere già pronti: account Google Cloud con billing abilitato (per Cloud Run/Secret Manager/Cloud Build), repository GitHub collegabile a Cloud Build, accesso alla console MongoDB Atlas, credenziali Resend già in uso in Fase 2 (per il valore di produzione di `RESEND_FROM_ADDRESS`, ancora da confermare — vedi `fase-3-deploy.md` § 3.2). Il progetto Google Cloud OAuth è lo stesso di sviluppo, ma con un Client OAuth dedicato alla produzione da creare (§ 3.2/3.3) — non serve un nuovo progetto GCP.
- **Fase 4**: Fase 3 chiusa (✅ su tutte le sottofasi in `fase-3-deploy.md`, deploy in produzione attivo). Prima di iniziare la sottofase relativa al sync HubSpot (Passo 3, bloccata dal Passo 0), avere già pronti: Service Key HubSpot con scope `crm.objects.contacts.read` e relativo token. Le altre sottofasi (schema dati, Global di configurazione, upload CSV, API Wildcard, endpoint verifica invito) non richiedono passaggi esterni propri.
- **Fase 5**: Fase 4 chiusa (✅ su tutte le sottofasi in `fase-4-import-sync.md`). Nessun passaggio esterno nuovo — nessuna credenziale o account da procurarsi, a differenza delle fasi precedenti. Riferimento decisionale: `specifica-reset-contatti-log.md` (sezione "Perimetro GDPR e decisioni collegate" per il Passo 5, documento operativo).
