...
- **Prossimo passo**: **Sviluppo App** (nav/shell, lista/scheda contatti ricche — oggi esiste solo la superficie minima Contatti del Passo 5) e/o sessioni di decisione sotto. Invio massivo reale resta bloccato da **due gate indipendenti**: upgrade billing Resend a Pro **e** flag `pianoResendPro` in Admin; fino all’evento mantenere `modalitaTestInvio` true e `contattiTest` solo indirizzi di test.
- **Sessioni di decisione ancora da tenere** (non implementazione, prima serve discutere): lettore di check-in (libreria scansione, endpoint validazione, gestione offline — da tenere dopo Fase 6, prima della relativa implementazione); design area pubblica (homepage neutra + pagina pubblica biglietto, `design-system.mdc` della landing evento fuori da questo repo, testi/copy coerenti con l'evento); **CMS per testi pagine pubbliche** (dove editare i contenuti guest-facing in Payload); **template email Payload vs Resend** (ticket/login oggi in codice — analizzare se spostare in CMS Payload o template Resend). Dettaglio in `fase-6-invio-ticket.md` §5.
- **Debiti rimasti aperti da Fase 4** (`fase-4-import-sync.md` § 3, non bloccanti per la chiusura): soft-match CSV senza email; Caso F soft-delete storici; ottimizzazione sync HubSpot (update/log selettivo); cleanup soft-delete; propagazione manuale `INVITE_API_KEY` su Firebase; `hubspotOwner` ID grezzo vs nome risolto; **`source` senza valore «Inserito da Admin»** (oggi solo Hubspot/Upload/Wildcard — emerso in verifica Fase 6 Passo 1). *(I debiti "post-insert Wildcard / thank-you / trigger ticket" e "`ticketConfig`" sono chiusi da Fase 6 — vedi `fase-6-invio-ticket.md` §2.0. Se `fase-4-import-sync.md` §3 non viene aggiornato di conseguenza, questo indice resta la fonte di verità sullo stato reale.)*
- **Debiti vincolanti per Sviluppo App** (dal test Wildcard Passo 5): form con `dudeCompany` select (SRL, Milano, London, Things, Design, Originals, Fondazione/MFF); campo telefono (schema + migrazione, verificare anche visibilità/editabilità in Admin); `assegnazione` auto da email utente App; all'insert `partyDude=SI` + `partyTtt=YES` server-side; **campi `wildcardQuota`/`wildcardUsed` su `users`** (quota per `appRole: manager`, illimitato per `full-access`, quota rimanente visibile in UI, bottone disabilitato a quota esaurita — decisioni in `analisi-vecchi-progetti-wa-wildcard.md`, non ancora elencata qui in precedenza) — dettaglio in `fase-4-import-sync.md` § 3.
