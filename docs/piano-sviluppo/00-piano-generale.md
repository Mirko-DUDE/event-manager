...
| Fase 7 | Area App UI/UX: shell, auth, lista/scheda contatti, wildcard, check-in (shadcn/ui + Tailwind) | 🔶 in corso (Passo 0–8 ✅, Passo 9 parziale — paginazione lista su Cloud Run 🔲; dataset prod ~30 contatti, test sbloccato) | `fase-7-area-app-ui.md` |
| Fase 8 | Contenuti reali evento: grafica e copy email ticket + pagina pubblica biglietto (per-evento, non riusabile as-is) | 🔶 in corso (Passo 0–3 ✅, pagina ✅, invio massivo test prod 30 contatti ✅; rendering 3 client 🔲) | `fase-8-contenuti-evento.md` |
| Fase 9 | Homepage pubblica (`/`): due bottoni verso App/Admin, identità corporate dude.it (stabile, non per-evento) | ✅ fatto (Passo 0–3 + verifica visiva mobile/desktop) | `fase-9-homepage-pubblica.md` |
| Fase 10 | Sicurezza, indicizzazione e scadenza pagina pubblica biglietto: `robots.txt`/meta robots, Referrer-Policy, scadenza esplicita ticket | ✅ fatto (Passo 0–6) | `fase-10-sicurezza-indicizzazione.md` |
...
Dettaglio completo in `fase-7-area-app-ui.md` (decisioni definite in sessione mockup dedicata, 14 file HTML approvati — 8 mobile, 6 desktop — in `preparazione-design-app.md`, **da committare nel repo se non già presente**, mockup in `docs/design/app-mockups/`, verificare se già tracciati; riferimento decisionale anche `specifica-lettore-checkin.md`, chiusa, e `fase-6-invio-ticket.md`, chiusa, la cui logica di invio/resend viene riusata senza modifiche). **Fase 7** — Passo 0–8 ✅, Passo 9 🔶 (resta paginazione lista su Cloud Run).
...
9. Verifica di chiusura fase — 🔶 *(2026-08-21: device reali, bottom nav hostess, `checkInUndo` Admin, access denied `appRole: none` OK; resta paginazione lista Passo 8 #4 su Cloud Run — dataset prod ~30 contatti dal 2026-08-25, test sbloccato)*

## Prossimi passi

- **Fase 9 — Homepage pubblica**: **chiusa** 2026-08-21 (Passo 0–3 + verifica visiva mobile/desktop ✅). Dettaglio in `fase-9-homepage-pubblica.md` §5.
- **Implementazione Fase 7 (Passi 0–8)**: committata; **Passo 9** quasi chiuso — resta **paginazione lista contatti** (checklist Passo 8 #4) su Cloud Run; dal 2026-08-25 il DB prod ha ~30 contatti attivi (Upload+Wildcard) → paginazione verificabile; vedi `fase-7-area-app-ui.md` Passo 9.
- **Fase 8 — Contenuti evento**: implementazione Passo 0–3 ✅; pagina pubblica mobile/desktop ✅ (2026-08-21); **invio massivo test produzione** 30 contatti ✅ (2026-08-25, Resend Free, `modalitaTestInvio` off — DB solo contatti test/interni); resta **verifica rendering email** su Gmail + Outlook + Apple Mail (logo CID, QR, copy DUDEHUB) — vedi `fase-8-contenuti-evento.md` §9. Insight Resend (link Maps vs dominio mittente, `noreply@`) valutati non bloccanti — nessuna modifica richiesta prima dell’evento salvo problemi deliverability in test.
- **Prossimo passo operativo (test umani)**: (1) paginazione Fase 7 su `/app/contatti` (prod); (2) chiusura checklist email Fase 8 su 3 client. **Prima di import/sync invitati reali**: riattivare `modalitaTestInvio` (o whitelist `contattiTest`). Invio massivo evento reale (~3000) resta bloccato da upgrade Resend Pro + `pianoResendPro`.
- **Fase 3 — Amendment CSRF dual-origin (2026-08-21)**: `CLOUD_RUN_URL` impostata su Cloud Run ✅. Smoke produzione: Google OAuth da `*.run.app` reindirizza a `events.dude.it` (atteso); login locale su `*.run.app` senza redirect (fallback emergenza). Vedi `fase-3-deploy.md` checklist post-deploy.
...
- **Fase 10 — Sicurezza, indicizzazione e scadenza pagina pubblica** — **chiusa** 2026-08-21 (Passo 0–6 ✅, inclusa verifica anteprima OG WhatsApp su produzione). Dettaglio in `fase-10-sicurezza-indicizzazione.md` §7.
...
- **Debito test Fase 7 Passo 9** *(2026-08-25)*: paginazione lista su Cloud Run (Passo 8 #4) — **unico residuo**; dataset prod ~30 contatti (Upload+Wildcard) sblocca il test. Resto Passo 9 ✅ (2026-08-21).
- **Debito test Fase 8 §9** *(2026-08-25)*: invio massivo 30 contatti test in prod ✅; resta verifica rendering su **Gmail, Outlook, Apple Mail** (checklist §9).
