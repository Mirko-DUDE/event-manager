...
- **Sessioni di decisione ancora da tenere** (non implementazione, prima serve discutere): design area pubblica (homepage neutra + pagina pubblica biglietto, `design-system.mdc` della landing evento fuori da questo repo, testi/copy coerenti con l'evento); **CMS per testi pagine pubbliche** (dove editare i contenuti guest-facing in Payload); **template email Payload vs Resend** (ticket/login oggi in codice — analizzare se spostare in CMS Payload o template Resend). Dettaglio in `fase-6-invio-ticket.md` §5.
