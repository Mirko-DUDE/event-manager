...
- **Form Wildcard — UX Area App (debiti vincolanti per Sviluppo App)** — emersi dal test dev Passo 5 (2026-08-05); **chiusi lato schema/logica server in Fase 7 Passo 0** (2026-08-07): vedi `fase-7-area-app-ui.md` §2.3/Passo 0 — fonte unica. Restano al Passo 6 UI: select `dudeCompany`, campo telefono nel form, badge quota, validazione email-or-phone.
  1. **`dudeCompany` come select** — valori in `lib/contacts/dudeCompany.ts`; schema `contatti.dudeCompany` resta text (compatibile sync HubSpot/CSV). UI select = Passo 6.
  2. **Campo `telefono`** — aggiunto su `contatti` (Admin + defaultColumns). Nessun mapping HubSpot. Form Wildcard = Passo 6.
  3. **`assegnazione` auto** — server-side da email utente (`deriveAssegnazioneFromEmail`); input client ignorato. UI read-only = Passo 6.
  4. **`partyDude` / `partyTtt`** — impostati server-side (`SI`/`YES`) in `insertWildcardContact`. ✅
  5. **Quota Wildcard** — `wildcardQuota`/`wildcardUsed` su `users`, enforcement `quotaEsaurita` per manager in `insertWildcardContact`. UI badge/bottone disabilitato = Passo 2/6.
...
- **`source` — valore per inserimento da Admin**: oggi l’enum `contatti.source` ha solo `Hubspot` / `Upload` / `Wildcard`. Un contatto creato a mano da Payload Admin resta senza source (o con un valore forzato a mano tra quelli esistenti, fuorviante). **Debito** (emerso in verifica Fase 6 Passo 1, 2026-08-07): aggiungere un’opzione dedicata (es. label «Inserito da Admin», value `Admin`) e, se utile, default/hook o `createdBy` coerente all’insert Admin. Non bloccante per Fase 6.
