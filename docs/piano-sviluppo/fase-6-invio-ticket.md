...
- **Design della pagina pubblica del biglietto e della homepage pubblica** — **risolto**: pagina biglietto disegnata e implementata in `fase-8-contenuti-evento.md` (Fase 8); homepage disegnata in `fase-9-homepage-pubblica.md` (Fase 9, 2026-08-20). Nessuna sessione di design ancora aperta su queste due superfici.
- **CMS per contenuti delle pagine pubbliche** (sessione dedicata, 2026-08-20 — chiude il debito aperto il 2026-08-07): **nessun CMS in questa versione**, per entrambe le superfici pubbliche.
  - **Pagina biglietto** (`/ticket/[qrToken]`): debito chiuso **strutturalmente**, non solo rimandato — il contenuto è la trasposizione in pagina del template email ticket (stessa decisione presa sopra per il template email: hardcoded, cambia per-evento). Non esiste un contenuto pagina-biglietto indipendente da gestire via CMS; eredita la stessa fonte di verità dell'email.
  - **Homepage pubblica** (`/`): resta una **pagina standard/generica** per questa versione, definita come ultimo aspetto tra le superfici pubbliche (priorità bassa, non bloccante per il resto). L'uso di Payload come CMS per i suoi contenuti è esplicitamente rimandato a uno sviluppo futuro, non a questa versione.
  - Nota: questa decisione riguarda solo la gestione editoriale dei contenuti (CMS sì/no); il design visivo (Fase 8 per il biglietto, Fase 9 per la homepage) è ora risolto separatamente, vedi bullet sopra.
- **Template email guest-facing: codice/Payload vs Resend** (sessione dedicata, 2026-08-20 — aggiorna il debito aperto il 2026-08-07): **decisione confermata per questa versione: restare hardcoded** (`renderTicketEmail.ts`, coerente col pattern già in uso in `auth/email/`). Nessuna introduzione di Payload (Global/collection con rich text) né di Resend Templates (dashboard/API) in questa fase.
  - Motivazione: per un solo evento gestito, non esiste un bisogno reale di editing senza redeploy da parte di personale non tecnico — unico beneficio che le due alternative offrirebbero. Introdurre struttura/astrazione ora, con un solo caso concreto davanti, significherebbe progettarla alla cieca (quali campi sono davvero variabili, vincoli su immagini/colori, ecc.) — stesso principio di "rimandare la struttura finché il pattern non emerge" già seguito altrove nel progetto.
  - **Informazione emersa in questa sessione, rilevante per la rivalutazione futura**: la grafica dell'email ticket (colori, testi, immagini) cambia **evento per evento**, pur mantenendo un layout simile — non è un contenuto stabile scritto una volta.
  - **Condizione esplicita che riapre la valutazione**: se Event Manager verrà riusato su un secondo evento reale, il criterio decisionale cambia da "frequenza di modifica" (qui: mai) a "personalizzazione ricorrente per-evento con layout stabile" — uno scenario in cui sia un Global Payload per evento sia un Resend Template per evento risolverebbero un problema reale (evitare di toccare codice e fare redeploy a ogni evento). Da rivalutare solo a quel punto, con il secondo evento come riferimento concreto per capire quali campi sono davvero variabili.
  - Nota tecnica da verificare se/quando si riaprirà la valutazione lato Resend: confermare che `template.id` + `variables` e `attachments` (necessario per il CID del QR, §2.6/2.1) coesistano nella stessa chiamata `POST /emails` — non verificato in questa sessione.
