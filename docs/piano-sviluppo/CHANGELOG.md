...
- **§ 3.3 — Cloud Run 500 su Payload/API**: `libvips-cpp.so.8.18.3` mancante su Alpine — Next standalone non segue il `dlopen()` del `.node` verso `@img/sharp-libvips-linuxmusl-x64`. Tre tentativi falliti (cp in root node_modules sbagliato; busybox cp con hard link CAS pnpm fallisce ENOENT; tar nel layer deps copiava nella posizione sbagliata — il RPATH del binario cerca libvips nella struttura `.pnpm`, non in `node_modules/@img`). Fix corretto: `outputFileTracingIncludes` in `next.config.ts` con glob `.pnpm/@img+sharp-libvips-linuxmusl-x64@*/**` — il tracer di Next include il pacchetto nello standalone mantenendo il path `.pnpm` che il RPATH si aspetta. Dockerfile ripristinato alla versione semplice senza copia manuale.
