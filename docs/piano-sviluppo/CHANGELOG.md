...
- **§ 3.3 — Cloud Run 500 su Payload/API**: `libvips-cpp.so.8.18.3` mancante su Alpine — Next standalone non traccia le `.so` di `@img/sharp-libvips-linuxmusl-x64`. Due tentativi falliti con `cp -r` in builder stage (primo: `node_modules/@img` non esiste in root con pnpm; secondo: busybox cp in Alpine ricrea hard link del CAS pnpm e fallisce ENOENT). Fix finale: estrazione con `tar -ch` (dereference) nel layer `deps` dove il CAS è presente e valido; COPY da deps a runner.
